...
import {
  Car, ShieldCheck, Check, Phone, ArrowRight, MapPin, Calendar,
  Clock, Plane, CreditCard, QrCode, AlertCircle, CheckCircle2, Ticket
} from 'lucide-react';
    ...
    <div className="space-y-12">
      ...
      <div className="bg-white rounded-[32px] border border-slate-200 shadow-lg p-6 sm:p-8 lg:p-10 space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
          ...
          <div className="flex items-center gap-1.5 bg-slate-100 p-1.5 rounded-full border border-slate-200 text-xs font-bold">
            <button
              type="button"
              onClick={() => setTripType('airport')}
              className={`px-4 py-2 rounded-full transition-all ${
                tripType === 'airport' ? 'bg-slate-900 text-white shadow-xs' : 'text-slate-700 hover:text-slate-900'
              }`}
            >
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full transition-all ${
                tripType === 'outstation' ? 'bg-slate-900 text-white shadow-xs' : 'text-slate-700 hover:text-slate-900'
              }`}
            >
        ...
        <form onSubmit={handleBookCab} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs sm:text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-slate-900"
                required
              />
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input
                  ...
                  className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-slate-900"
                />
                <input
                  type="text"
                  value={pickupTime}
                ...
                return (
                  <div
                    ...
                    className={`p-5 rounded-2xl border-2 cursor-pointer transition-all ${
                      isSelected
                        ? 'border-slate-950 bg-slate-50/80 shadow-md'
                        : 'border-slate-200 hover:border-slate-300 bg-white'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-bold uppercase tracking-wider text-slate-500">{cab.type}</span>
          ...
          <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-700">Passenger & Boarding Details</h4>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <input
                ...
                className="p-2.5 rounded-xl bg-white border border-slate-200 text-xs font-semibold"
                required
              />
              <input
                type="text"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
            ...
            <button
              ...
              className="w-full sm:w-auto px-8 py-3.5 rounded-full bg-slate-950 hover:bg-black text-white font-bold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2"
            >
              {isProcessing ? (
      ...
      {isModalOpen && confirmedBookingId && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6">
          <div className="bg-white rounded-[32px] max-w-md w-full border border-slate-200 shadow-2xl p-6 sm:p-8 space-y-6 text-slate-900 animate-fade-in text-center">
            ...
            <div className="space-y-2">
              ...
              <div className="inline-block px-4 py-2 rounded-xl bg-slate-100 font-mono font-bold text-base text-slate-900 border border-slate-200">
                {confirmedBookingId}
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-left text-xs space-y-2">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-900 truncate max-w-[200px]">{pickup}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Destination:</span>
                <span className="font-semibold text-slate-900 truncate max-w-[200px]">{dropoff}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Vehicle:</span>
                <span className="font-semibold text-slate-900">{selectedCab.name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Total Tariff:</span>
                <span className="font-bold text-emerald-700">₹{totalTariff.toLocaleString('en-IN')} (Tolls Included)</span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <a
                href="/my-bookings"
                className="w-full py-3 px-4 rounded-full bg-slate-950 text-white font-bold text-xs flex items-center justify-center gap-2 hover:bg-black"
              >
              ...
              <button
                ...
                className="w-full py-2.5 px-4 rounded-full bg-slate-100 text-slate-700 font-semibold text-xs hover:bg-slate-200"
              >
                Close
              </button>
            </div>
